...
// ---------------- Fault Simulation ----------------
static void decode_to_nibbles(const unsigned char *in, int len,
...
// static void encode_from_nibbles(unsigned char *sig,
//                                 const unsigned char *nibbles, int n) {
//   memset(sig, 0, (n + 1) / 2);
//   for (int i = 0; i < n; i++) {
//     if (i & 1)
...
static int example_fault_sim(const mayo_params_t *p) {
  ...
  printf("Checking if fault is ineffective (signature verifies)\n");
  ...
  if (res == MAYO_OK) {
    recovered = orig; // Fault ineffective, s[0] = (O x)[0]
  } else {
    recovered = 0xFF; // Fault effective, cannot recover
  }
